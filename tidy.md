...
The functions `pivot_longer()` and `pivot_wider()` are not perfectly symmetrical because column type information is when a data frame is converted from wide to long.
...
Both arguments accept named lists to set set values for each column.
